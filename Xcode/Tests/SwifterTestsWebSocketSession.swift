...
class SwifterTestsWebSocketSession: XCTestCase {
  ...
  func testParser() {
    ...
    do {
      ...
      let frame = try session.readFrame()
      XCTAssertEqual(
        frame.opcode, WebSocketSession.OpCode.continue, "Parser should accept Continue opcode.")
    } catch {
    ...
    do {
      ...
      let frame = try session.readFrame()
      XCTAssertEqual(
        frame.opcode, WebSocketSession.OpCode.text, "Parser should accept Text opcode.")
    } catch {
    ...
    do {
      ...
      let frame = try session.readFrame()
      XCTAssertEqual(
        frame.opcode, WebSocketSession.OpCode.binary, "Parser should accept Binary opcode.")
    } catch {
    ...
    do {
      ...
      let frame = try session.readFrame()
      XCTAssertEqual(
        frame.opcode, WebSocketSession.OpCode.close, "Parser should accept Close opcode.")
    } catch let error {
    ...
    do {
      ...
      let frame = try session.readFrame()
      XCTAssertEqual(
        frame.opcode, WebSocketSession.OpCode.ping, "Parser should accept Ping opcode.")
    } catch let error {
    ...
    do {
      ...
      let frame = try session.readFrame()
      XCTAssertEqual(
        frame.opcode, WebSocketSession.OpCode.pong, "Parser should accept Pong opcode.")
    } catch let error {
    ...
    for opcode in [3, 4, 5, 6, 7, 11, 12, 13, 14, 15] {
      do {
        ...
        XCTAssert(
          false, "Parse should throw UnknownOpCode error for unknown opcode (was \(error)).")
      }
    }
  }
}
