...
class ServerThreadingTests: XCTestCase {
    ...
    do {
      ...
      (1...3).forEach { index in
        queue.asyncAfter(deadline: .now() + .seconds(index)) {
          let task = URLSession.shared.executeAsyncTask(hostURL: hostURL, path: path) {
            (_, response, _) in
  ...
  func testShouldHandleTheSameRequestConcurrently() {
    ...
    var requestExpectation: XCTestExpectation? = expectation(
      description: "Should handle the request concurrently")
