...
class SwifterTestsStringExtensions: XCTestCase {
  ...
  func testBASE64() {
    ...
    XCTAssertEqual(
      String.toBase64([UInt8]("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx".utf8)),
  ...
  func testMiscTrim() {
    ...
    XCTAssertEqual(
      "   \t\n\rtest  n   \n\t asd    ".trimmingCharacters(in: .whitespacesAndNewlines),
      "test  n   \n\t asd")
  }
  ...
  func testMiscReplace() {
    ...
    XCTAssertEqual("+++".replacingOccurrences(of: "+", with: "-"), "---")
    XCTAssertEqual(
      "t&e&s&t12%3%".replacingOccurrences(of: "&", with: "+").replacingOccurrences(
        of: "%", with: "+"), "t+e+s+t12+3+")
    XCTAssertEqual(
      "test 1234 #$%^&*( test   ".replacingOccurrences(of: " ", with: "_"),
      "test_1234_#$%^&*(_test___")
  }
