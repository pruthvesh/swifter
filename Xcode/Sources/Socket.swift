...
open class Socket: Hashable, Equatable {
  ...
  private func writeBuffer(_ pointer: UnsafeRawPointer, length: Int) throws {
    ...
    while sent < length {
      #if os(Linux)
        let result = send(
          self.socketFileDescriptor, pointer + sent, Int(length - sent), Int32(MSG_NOSIGNAL))
      #else
  ...
  func read(into buffer: inout UnsafeMutableBufferPointer<UInt8>, length: Int) throws -> Int {
    ...
    while offset < length {
      ...
      let readLength =
        offset + Socket.kBufferLength < length ? Socket.kBufferLength : length - offset

      #if os(Linux)
        let bytesRead = Glibc.read(
          self.socketFileDescriptor as Int32, baseAddress + offset, readLength)
      #else
  ...
  public func peername() throws -> String {
    ...
    if getnameinfo(&addr, len, &hostBuffer, socklen_t(hostBuffer.count), nil, 0, NI_NUMERICHOST)
      != 0
    {
  ...
  public class func setNoSigPipe(_ socket: Int32) {
      ...
      setsockopt(
        socket, SOL_SOCKET, SO_NOSIGPIPE, &no_sig_pipe, socklen_t(MemoryLayout<Int32>.size))
    #endif
  }
